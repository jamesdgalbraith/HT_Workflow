...
# run rpstblastn
system("rpstblastn -db ~/Databases/localrpsb/db/Pfam -outfmt \"6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen\" -evalue 0.01 -query raw_data/candidate.fa -out raw_data/candidate_pfam.out")
system("rpstblastn -db ~/Databases/localrpsb/db/Cdd_NCBI -outfmt \"6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen\" -evalue 0.01 -query raw_data/candidate.fa -out raw_data/candidate_cdd.out")

# read in rpstblastn data
rpstblastn_out <- read_tsv("raw_data/candidate_cdd.out", col_names = c("seqnames", "sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore", "qlen", "slen")) %>%
  rbind(read_tsv("raw_data/candidate_pfam.out", col_names = c("seqnames", "sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore", "qlen", "slen"))) %>%
...
# determine sequences containing both RT and EN
