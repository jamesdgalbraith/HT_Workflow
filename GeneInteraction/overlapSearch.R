...
aipysurus_repeat_annotation_corrected %>%
  dplyr::filter(grepl("Snek", m_repeat))
...
write_tsv(relevant_snek_insertions, "GeneInteraction/relevant_insertions.tsv")

# manually determine gene names, read back in
relevant_snek_insertions_fixed <- read_tsv("GeneInteraction/relevant_insertions_fixed.tsv") %>%
  mutate(appended_names = paste0(gene, "#", m_repeat))

relevant_snek_insertions_ranges <- plyranges::as_granges(relevant_snek_insertions_fixed)
...
names(relevant_snek_insertions_seq) <- relevant_snek_insertions_ranges$appended_names
