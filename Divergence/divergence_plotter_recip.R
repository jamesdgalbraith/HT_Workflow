...
repeat_list <- tibble(repeat_name = c("Proto2-Snek", "Rex1-Snek_1", "Rex1-Snek_2", "Rex1-Snek_3", "Rex1-Snek_4", "RTE-Snek", "RTE-Snek_2"),
                      count_max = c(300, 30, 120, 80, 100, 125, 120),
                      bp_max = c(120000, 10000, 20000, 17000, 30000, 41000, 175000))
...
for(i in 1:nrow(species_list)){
  ...
  blastn <- read.table(text=system(paste0("blastn -dust yes -query ~/HT_Workflow/Divergence/compiled_LINEs.fasta -db ", genome_path, " -outfmt \"6 sseqid sstart send pident qcovs bitscore length mismatch evalue qseqid\""), intern = TRUE), col.names = c("seqnames", "sstart", "send", "pident", "qcovs", "bitscore", "length", "mismatch", "evalue", "qseqid")) %>%
    as_tibble() %>%
  ...
  for(j in 1:nrow(repeat_list)){
    ...
    if(nrow(bed_tbl)>0){
      ...
      blastn_recip <- read.table(text=system(paste0("blastn -dust yes -query ~/HT_Workflow/Divergence/scratch/temp.fa -subject ~/HT_Workflow/Divergence/compiled_LINEs.fasta -outfmt \"6 sseqid sstart send pident qcovs bitscore length mismatch evalue qseqid qlen slen\""), intern = TRUE), col.names = c("seqnames", "sstart", "send", "pident", "qcovs", "bitscore", "length", "mismatch", "evalue", "qseqid", "qlen", "slen")) %>%
        as_tibble() %>%
        dplyr::filter(length >= 50) %>%
        mutate(pident_rnd = ceiling(pident), div = 100 - pident_rnd, d = mismatch/length, seqnames = as.character(seqnames), qseqid = as.character(qseqid),
               jc_dist = (-3 / 4) * log(1 - (4 * d / 3))) # calculate Jukes-Cantor distance
    ...
    bp_coverage <- matrix(rep(0, length(blastn_recip$seqnames)*as.numeric(cons_len)), byrow = T, ncol = as.numeric(cons_len))
    
    # calculate coverage of each base pair
    for(k in 1:length(blastn_recip$seqnames)){
